...
help:
	...
	@printf '%s\n' '  STREAM_HOST=<receiver-ip> make run-pose-control-sitl-pi  SITL UDP: stable DOWN gestures command RTL via MAVLink'
	@printf '%s\n' '  scripts/stream-to-rx.sh <receiver-ip>'
	@printf '%s\n' '  make run-rx'
	@printf '%s\n' '  make pixhawk-bench-gate-test-pi  Read-only Pixhawk UART gate check for injected DOWN-to-RTL triggers'
	...
	@printf '%s\n' '  make sitl-gesture-control-test-pi  SITL-only DOWN trigger to RTL mode-change test'
	@printf '%s\n' ''
	@printf '%s\n' 'Common overrides:'
